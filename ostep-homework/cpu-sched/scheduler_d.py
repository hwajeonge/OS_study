...
def execute_rr(joblist, quantum):
    ...
    start_times = {}
    completion_times = {}
    arrival_times = {}  # 도착 시간을 저장할 사전
    running_job = None
    queue_entry_times = {}  # 각 작업이 준비 큐에 들어간 시간을 저장
    wait_times = {}  # 각 작업의 총 대기 시간을 저장
    ...
    while joblist or ready_queue or running_job:
        ...
        while joblist and joblist[0][2] <= current_time:
            ...
            queue_entry_times[new_job[0]] = current_time  # 큐에 들어간 시간 기록
        ...
        if not running_job and ready_queue:
            ...
            if running_job[0] not in wait_times:
                wait_times[running_job[0]] = 0
            wait_times[running_job[0]] += current_time - queue_entry_times[running_job[0]]
        ...
        if running_job:
            ...
            if running_job[1] <= 0:
                ...
                ready_queue.append(running_job)
                queue_entry_times[running_job[0]] = current_time  # 다시 큐에 들어간 시간 업데이트
