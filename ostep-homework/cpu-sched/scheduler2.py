...
if options.solve:
    ...
    if options.policy == 'SJF' and options.arrivals:
        ...
        while runlist:
            for job in runlist[:]:  # Iterate a copy of the runlist to allow removal inside the loop
                if thetime < job[2]:  # If the job hasn't arrived yet
                    thetime = max(thetime, job[2])  # Wait for the job to arrive
                print(f'  [ time {thetime:3d} ] Run job {job[0]} for min({options.quantum}, {job[1]}) secs')
                executed_time = min(options.quantum, job[1])
                job[1] -= executed_time  # Decrease the remaining time by the executed time
                thetime += executed_time

                if job[1] <= 0:  # If the job is completed
                    print(f'  [ time {thetime:3d} ] Finish job {job[0]}')
                    completion_times[job[0]] = thetime
                    runlist.remove(job)  # Remove the job from the runlist
                else:
