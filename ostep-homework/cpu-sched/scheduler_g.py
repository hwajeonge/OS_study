...
def parse_options():
    ...
    parser.add_option("-a", "--arrival_time", default="", help="comma-separated list of arrival times for each job", action="store", type="string", dest="arrival_time") #도착 시간 지정 옵션 추가
...
def main():
    ...
    if options.solve:
        if options.policy == 'SJF':
            #SJF로 선택되었고, 도착시간이 지정되면 STCF 함수로 실행
            if options.arrival_time:
...
def execute_fifo(joblist): #도착시간 고려사항 추가
    print('** Executing FIFO scheduling **\n')
    thetime = 0
    job_responses = {}
    start_times = {}
    completion_times = {}
    joblist.sort(key=lambda x: x[2])  # 도착시간으로 정렬

    for job in joblist:
        if thetime < job[2]:  
            thetime = job[2]
        if job[0] not in job_responses:
            job_responses[job[0]] = thetime - job[2]  
...
def execute_rr(joblist, quantum): #도착 시간 고려사항 추가
...
# * 실행한 명령어
# python3 -p RR -j 3 -s 100 -q 1 -c
# python3 -p RR -j 3 -s 100 -q 1 -c -a "0, 2, 4"
# python3 -p FIFO -j 3 -s 100 -q 1 -c
# python3 -p FIFO -j 3 -s 100 -q 1 -c -a "0, 2, 4"
# python3 -p SJF -j 3 -s 100 -q 1 -c
# python3 -p SJF -j 3 -s 100 -q 1 -c -a "0, 2, 4"
