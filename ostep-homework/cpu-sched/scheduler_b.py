...
def execute_stcf(joblist):
    ...
    while joblist or ready_queue or running_job:
        # Add jobs to the ready queue based on arrival time
        ...
        if not running_job or (ready_queue and ready_queue[0][0] < remaining_times[running_job[0]]):
            ...
            if ready_queue:
                ...
                if running_job[0] not in first_run:
                    first_run[running_job[0]] = current_time - running_job[2]  # Mark first run for response time calculation
    ...
    for jobnum in first_run:
        response_time = first_run[jobnum]  # Get the first run time for response
        completion = completion_times[jobnum]
        wait_time = start_times[jobnum] - joblist[jobnum][2]
        turnaround_time = completion - joblist[jobnum][2]
    ...
    while joblist or ready_queue or running_job:
        ...
        if not running_job and ready_queue:
            ...
            if running_job[0] not in job_responses:
                job_responses[running_job[0]] = current_time - running_job[2]
            start_times[running_job[0]] = current_time
